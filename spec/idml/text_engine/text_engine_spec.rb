...
RSpec.describe Idml::TextEngine::VerticalLayout do
  it "positions glyphs on the same line with identical Y" do
    frame = described_class::Frame.new(72, 720, 468, 648, 0, 0, 0, 0)
    line = Idml::TextEngine::Line.new(
    ...
    expect(result.first.y).to eq(result.last.y)
    expect(result.first.x).to be < result.last.x
  end

  it "positions glyphs on different lines with descending Y" do
    frame = described_class::Frame.new(72, 720, 468, 648, 0, 0, 0, 0)
    line1 = Idml::TextEngine::Line.new(
      [Idml::TextEngine::ShapedGlyph.new("A".ord, 7, false)], 7, 0
    )
    line2 = Idml::TextEngine::Line.new(
      [Idml::TextEngine::ShapedGlyph.new("B".ord, 7, false)], 7, 0
    )
    result = described_class.layout(
      lines: [line1, line2],
      frame: frame,
    ...
    expect(result.first.y).to be > result.last.y
  end
end
